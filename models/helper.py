...
def get_backbone(backbone_arch='resnet50',
    ...
    """
    if 'musc' in backbone_arch.lower():
        return backbones.ResNetMusc(backbone_arch, pretrained, layers_to_freeze, layers_to_crop)
...
def get_aggregator(agg_arch='ConvAP', agg_config={}):
    ...
    elif 'muscvpr' in agg_arch.lower():
        assert 'in_channels' in agg_config
        assert 'out_channels' in agg_config
        assert 'in_h' in agg_config
        assert 'in_w' in agg_config
        assert 'in_h_1' in agg_config
        assert 'in_w_1' in agg_config
        assert 'mix_depth' in agg_config
        return aggregators.MuscVPR(**agg_config)